...
class Attention(nn.Module):
    ...
    def init_forward_attn(self, memory):
        B = memory.size(0)
        T = memory.size(1)
        alpha_ones = torch.ones([B, 1])
        alpha_zeros = torch.zeros([B, T - 1])
        alpha_zeros[:, :] = 1e-7
        self.alpha = torch.cat([alpha_ones, alpha_zeros], dim=-1).to(memory.device)
        self.u = (0.5 * torch.ones([B, 1])).to(memory.device)

    def apply_windowing(self, alignment, memory):
        ...
        self.forcing_idx.append(self.win_idx)
        return alignment

    def apply_forward_attn(self, alignment, fill_mask=False):
        forward_shifted_alpha = F.pad(self.alpha[:, :-1].clone().to(alignment.device), (1, 0, 0, 0))
        alpha = ((1 - self.u) * self.alpha + self.u * forward_shifted_alpha + 1e-8) * alignment

        if fill_mask:
            _, n = forward_shifted_alpha.max(1)
            val, _ = alpha.max(1)

            for b in range(alignment.size(0)):
                alpha[b, n[b] + 3:] = 0
                alpha[b, :(n[b] - 1)] = 0
    ...
    def inference(self, attention_hidden_state, memory, processed_memory,
        ...
        attention_weights = self.apply_forward_attn(attention_weights, fill_mask=True)
...
class Decoder(nn.Module):
    ...
    def inference(self, memory, input_sequence):
        ''' Decoder inference
        PARAMS
        ------
        memory: Encoder outputs
        ...
        # attention_hidden, attention_cell, decoder_hidden, decoder_cell, attention_weights,
        # attention_weights_cum, attention_context, processed_memory, mask
        states = self.get_initailize_decoder_states(memory, mask=None)
        self.attention_layer.init_window()
        self.attention_layer.init_forward_attn(memory)
        # att_h, att_c, dec_h, dec_c, att_w, att_w_cum, att_c, proc_mem, _
        ...
        # print(self.attention_layer.forcing_idx)
        if len(self.attention_layer.forcing_idx) > 0:
            print()
            from text import sequence_to_text
            from text.custom_symbols import _JAMO_TAILS, _JAMO_VOWELS
            ...
            # print([text_sequence[i] for i in self.attention_layer.forcing_idx])

            step = 5
            max_len = len(self.attention_layer.forcing_idx)
            for i in range(0, max_len,  step):
                for j in range(step):
                    if i + j == max_len:
                        break

                    print(text_sequence[self.attention_layer.forcing_idx[i + j]], end=' ')

                end = i + step - 1
                end = end if end < max_len else max_len
                print('{} ~ {}'.format(i, end))

            print()
            print('**** check attention fail')
            prev_idx = self.attention_layer.forcing_idx[0]
            for index, idx in enumerate(self.attention_layer.forcing_idx[1:]):
                if idx < prev_idx:
                    print('prev idx : {}, current idx : {}, index : {}'.format(prev_idx, idx, index), end='\t')
                    print('prev jamo : {}, current jamo : {}'.format(text_sequence[prev_idx], text_sequence[idx]))
