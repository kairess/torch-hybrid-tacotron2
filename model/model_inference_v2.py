...
class Attention(nn.Module):
    ...
    def init_window(self):
        ...
        win_back = 2
        win_front = 2
        return win_idx, win_back, win_front

    def apply_windowing(self, alignment, memory, win_idx, win_back, win_front, forcing_idx=None):
        back_win = win_idx - win_back
        back_win = back_win if back_win > 0 else 0
        front_win = win_idx + win_front

        if forcing_idx is not None:
            alignment[:, forcing_idx] *= 2
            back_win = forcing_idx - win_back
            front_win = forcing_idx + win_front

        if front_win < memory.size(1):
            alignment[:, back_win:front_win] *= 1.2

        else:
            alignment[:, back_win:] *= 1.2

        win_idx = torch.argmax(alignment, 1).long()[0].item()
        return alignment, win_idx
    ...
    def inference(self, attention_hidden_state, memory, processed_memory,
                attention_weights_cat, win_idx, win_back, win_front, forcing_idx=None, mask=None):
        ...
        alignment, win_idx = self.apply_windowing(alignment, memory, win_idx, win_back, win_front, forcing_idx)
        ...
        return attention_context, attention_weights, win_idx
...
class Decoder(nn.Module):
    ...
    def decode_infer(self, decoder_input , decoder_hidden, decoder_cell,
                        ...
                        memory, processed_memory, win_idx, win_back, win_front, forcing_idx):
        ...
        attention_context, attention_weights, win_idx = self.attention_layer.inference(attention_hidden, memory,
                                                                              processed_memory, attention_weights_cat,
                                                                              win_idx, win_back, win_front,
                                                                              forcing_idx, None)
        ...
        return decoder_output, gate_prediction, attention_weights, win_idx, (
            attention_hidden, attention_cell, decoder_hidden, decoder_cell, attention_weights, attention_weights_cum,
            attention_context)
    ...
    def inference(self, memory, input_sequence, postnet_layer):
        ''' Decoder inference
        PARAMS
        ------
        memory: Encoder outputs
        ...
        prev_argmax = 0
        win_idx, win_back, win_front = self.attention_layer.init_window()
        duration = []
        ...
        max_window = 15
        topk_window = 5
        topk_window = topk_window if max_seq_len >= topk_window else max_seq_len

        text_sequence = to_arr(input_sequence)[0].astype(np.int16).tolist()
        text_sequence = sequence_to_text(text_sequence) + '~'

        while True:
            decoder_input = self.prenet(decoder_input)
            mel_output, gate_output, alignment, win_idx, prev_states = self.decode_infer(decoder_input, decoder_hidden,
                                                                                         decoder_cell,
                                                                                         attention_hidden,
                                                                                         attention_cell,
                                                                                         attention_weights,
            ...
            current_argmax = torch.argmax(alignment, dim=1).item()
            topk_argmax = torch.topk(alignment, dim=1, k=topk_window)[1][0]

            if prev_argmax <= current_argmax and (any(topk_argmax == prev_argmax) or any(topk_argmax == prev_argmax + 1)):
                duration += [current_argmax]
                ...
                if current_argmax <= prev_argmax + 1:
                    prev_argmax = current_argmax

                elif any(topk_argmax == prev_argmax + 1):
                    if any(topk_argmax == prev_argmax):
                        prev_argmax_index = (topk_argmax == prev_argmax).nonzero().item()
                        front_argmax_index = (topk_argmax == prev_argmax + 1).nonzero().item()
                        if front_argmax_index < prev_argmax_index:
                            prev_argmax += 1
                    else:
                        prev_argmax += 1

                if len(duration[-max_window:]) == max_window and all([prev_argmax == argmax for argmax in duration[-max_window:]]):
                    forcing_idx = prev_argmax + 1 if prev_argmax < max_seq_len - 1 else None
            ...
            elif forcing_idx is not None and forcing_idx >= max_seq_len:
